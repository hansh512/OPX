...
$logFileDir=[System.IO.Path]::Combine($localPath,'Logs',(([System.Security.Principal.WindowsIdentity]::GetCurrent().Name).Replace('\','_')));
if (! (Test-Path -Path $logFileDir)) # verify if directory for log file exist
{
    New-Item -Path ([System.IO.Path]::Combine($localPath,'Logs')) -ItemType Directory -Name (([System.Security.Principal.WindowsIdentity]::GetCurrent().Name).Replace('\','_')) -Force;
}; # end if
