    ...
    process {                
        ...
        if ($PSBoundParameters.ContainsKey('IncludeAttributesFromConfigFile'))
        {
            try {
                ...
                $vDirCfgFile=$__OPX_ModuleData.VirtualDirCfgFileName;
    ...
    process {
        ...
        for ($j=0; $j -lt $MailCountToSend; $j++)
        {
            ...
            if ($SMTPServer -ne $false)
            {
                ...
                $wpParams.Status=('Sending test mail ' + $cMc + '/' + $MailCountToSend + '  SMTP server: ' + $SMTPServer + '  Sender: ' + $mailSendParams.From + ' Recipient: ' + $mailSendParams.To);
...
#region calulate namespces    
        ...
        {
            try {
                $ConfigurationFilePath=$__OPX_ModuleData.VirtualDirCfgFileName;
...
param([Parameter(ParameterSetName='ClearService',Mandatory = $true, Position = 0)][ValidateSet('OWA','ECP','OAB','Webservices','ActiveSync','OutlookAnywhere','Mapi','Autodiscover')][string]$Servcice,
    ...
    process {
        ...
        $cfgFilePath=$__OPX_ModuleData.VirtualDirCfgFileName;
...
param([parameter(Mandatory = $true)]
      ...
      [ArgumentCompleter( {
            ...
            $cfg=Import-Clixml -Path ($__OPX_ModuleData.VirtualDirCfgFileName);
            $ParameterPerService = @{
...
param([Parameter(ParametersetName='OU',Mandatory = $true, Position = 0)][AllowEmptyString()][string]$OrganizationalUnit,
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'OU')
        {
            ...
            writeTolog -LogString $msg;
            try {
                $val=[math]::round([System.Linq.Enumerable]::Sum([int[]]((((Import-csv -Path $CSVInputFilePath -ErrorAction Stop | Get-MailboxStatistics -ErrorAction Stop).TotalItemSize.Value) | %{$_.ToString().split('(')[1].split(' ')[0].replace(',','')})))/1GB,2);
                Write-Host ($val.ToString() + ' GB');
            } # end try
            catch
            {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUSB/obuaZYXrKPjjjSBzgFFpO
# fdugggXxMIIF7TCCBNWgAwIBAgITGwAAAEVNRiQcpqxVgwAAAAAARTANBgkqhkiG
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBSo0G3Qmnb562q9
# C9GTzHYCqrZqzDANBgkqhkiG9w0BAQEFAASCAQCSLazlmKpFCCudV66K/A9zCc00
# WGnlczu+HsgRn4mFIScKzGfebohKa122pTBqAhVk4oOTEmpKPIoL6Ql7yJBQX5Nz
# ml1Yojd2TtkMSjO5R5iS+1XX8P/gCSRzgVcU9+igyO2eD9plsx8I1jSikWmSChA7
# wZc2+CGngP7Juq17MJ7aNMKPFMST4dKyoUz9QmB6qN47rlXbsjm3rubJiWsgNEGp
# ljrIEtzRaA7hKfOloi/en6SoeFgVBPuzu8wQd6r90GGDGcZkoOBRewDUjoq4nGM0
# V+QiicRqhl6oP+Hs+39Y0cVGnDUXXpTPv5ufSFwcTzYL9XI861fZtcEU3Ykf
